...
let problem8() =
    let digits =  List.ofSeq <| Seq.map (fun c -> int c - int '0') ""
    let maxProductOfN1 digitList =  
        Seq.take ((List.length digitList) - 5)  digitList
            |> Seq.mapi (fun i _ -> digitList |> Seq.skip i |> Seq.take 5 |> Seq.reduce (*)) 
            |> Seq.max  
   
    maxProductOfN1 digits |> printfn "Problem 8  = %i" //40824

let problem8_fast() =
    let digits =  List.ofSeq <| Seq.map (fun c -> int c - int '0') ""
    let maxProductOfN digitList =
        let rec maxProductOfN' prev prevProduct maxProduct = function
            | lst when (List.length lst) < 5 -> maxProduct
            | x0::(x1::x2::x3::x4::xs as t) -> 
                let product = if prevProduct > 0 then prevProduct * x4 / prev 
                                else x0*x1*x2*x3*x4
                maxProductOfN' x0 product (max product maxProduct) t
        maxProductOfN' -1 -1 -1 digitList 

    maxProductOfN digits |> printfn "Problem 8 (fast) = %i" //40824

let problem9() =
    let getA b = (500000-1000*b)/(1000-b) 
    let getC b = 1000-b-getA b
    let isTriplet b = let a = getA b in let c = getC b in (a*a+b*b=c*c)
    [499..-1..200] |> List.find isTriplet |> fun b->(getA b)*b*(getC b) |> printfn "Problem 9 = %i"
